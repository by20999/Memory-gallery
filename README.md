...
`family-photo-gallery` 是一个面向个人、朋友、亲密关系或小团队的轻量回忆相册网站。项目使用原生前端和 Express 后端实现照片上传、浏览、分组、评论、表情回应、收藏、故事流和缩略图生成。
...
- 照片上传：支持多图上传、拖拽上传、本地预览、原图优先上传和必要时高质量处理。
...
- 回忆互动：支持昵称、本地自定义头像、评论、表情回应和照片故事说明。
