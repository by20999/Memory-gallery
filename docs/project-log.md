...
- 完成：将中速设为历史体感默认 `56px/s`，低速和高速分别拉开为更明显的实际滚动速度，切换档位时如果正在自动播放会立即重排动画帧并按新档位继续滚动。
- 验证：执行 `node --check js/story.js`、`node --check js/dom.js`、`npm run test:smoke`、`npm run audit:data` 通过；临时以 `PORT=3226` 启动 `node server.js` 后，首页 `/`、`/api/photos`、`/api/stories` 均返回 200，`/api/photos` 返回 128 条 JSON；确认旧的 `storyShowcaseSpeedInput`、速度范围常量和全屏速度 range 控件已无运行代码残留。
- 风险与后续：本轮未修改 API、数据字段、存储格式或真实上传数据；当前环境无 Browser 插件，尝试通过系统 Chrome 做 CDP 交互验证时 Chrome 复用了普通用户 profile，已停止该路径并清理临时进程，因此未完成截图级浏览器回归。

## 2026-06-19

- 完成：优化故事模式时间流线预览框的大跨度排序体验。拖动故事节点靠近时间流线左/右边缘时，预览框会自动横向滚动，并把滚动距离纳入排序目标计算，支持从前几张直接拖到后面较远的位置。
- 完成：将故事节点序号改为可编辑输入框，输入 `1-当前故事图片数` 范围内的数字后按回车或离开输入框即可跳转到对应位置；超出范围或无效输入会恢复原序号并提示用户。
- 验证：执行 `node --check js/story.js` 通过；本轮未修改 API、数据字段、存储格式或真实上传数据。

## 2026-06-19

- 完成：修正故事全屏花瓣从标题下方才出现的问题。`js/story.js` 在渲染后把 `story-showcase-static-layer` 从可拖动时间线轨道移到全屏展示根容器下，避免花瓣层受轨道位置和纵向 transform 影响。
- 完成：`style.css` 增加最终覆盖规则，将全屏静态氛围层、星尘、粒子和樱花花瓣固定为 `100vw x 100vh` 的视口层，使樱花从屏幕顶端上方开始下落，而不是从标题/时间线舞台下方出现。
- 验证：执行 `node --check js/story.js`、`npm run test:smoke`、`npm run audit:data` 通过；数据体检当前为 122 张照片、122 张缩略图、1 个故事、5 条故事引用、无断链。

## 2026-06-19

- 完成：继续按用户反馈增强电脑端故事全屏樱花氛围。桌面端花瓣节点从 28 片提升到 44 片，并整体提高默认、偶数、三倍数和五倍数花瓣透明度；新增第 29-44 片的横向分布、延迟、速度和尺寸差异，让下落密度更明显但仍保持错落感。
- 完成：移动端仍隐藏第 21 片之后的花瓣，并保留较低透明度，避免小屏遮挡照片、时间和文案。
- 验证：执行 `node --check js/story.js`、`npm run test:smoke`、`npm run audit:data` 通过；数据体检当前为 122 张照片、122 张缩略图、1 个故事、5 条故事引用、无断链。

## 2026-06-19

- 完成：按用户反馈增强故事模式全屏展示的樱花花瓣氛围。`js/story.js` 将全屏花瓣节点从 12 片提升为 28 片，并改为常量集中控制；`style.css` 追加最终覆盖块，补充分布、延迟、速度、大小和透明度，让花瓣在桌面端覆盖更多横向区域且下落节奏更错落。
- 完成：移动端保留前 20 片花瓣，避免小屏全屏展示时装饰过密遮挡照片、时间和文案；`prefers-reduced-motion` 既有降级规则继续生效。
- 验证：执行 `node --check js/story.js`、`npm run test:smoke`、`npm run audit:data` 通过；数据体检当前为 122 张照片、122 张缩略图、1 个故事、5 条故事引用、无断链。临时以 `PORT=3219` 启动 `node server.js` 后，首页 `/` 返回 200，`/api/photos` 返回 200 和 122 条 JSON。
- 风险与后续：本轮只调整全屏展示装饰密度和样式，没有修改 API、数据字段、存储格式或真实上传数据；未做浏览器截图级视觉回归，如继续精修全屏氛围，建议补一轮桌面与移动视口截图检查。

## 2026-06-18

- 完成：修复故事模式全屏展示中图片描述被截断的问题。`getShowcaseTitle()` 不再按标点删除尾部内容或强制截取 12 个字符，因此类似 `“我守望着。”` 的中文引号和句号会完整保留。
- 完成：为全屏描述样式增加最终兜底规则，取消标题和补充描述的 `line-clamp`、隐藏溢出和省略裁切，扩大描述容器宽度并允许正常换行，避免后续较长中文描述、引号、句号或其它尾部符号再次被吃掉。
- 验证：`node --check js/story.js` 通过；`npm run test:smoke` 通过；`npm run audit:data` 通过，当前 122 张照片、122 张缩略图、1 个故事、5 条故事引用、无断链；复用本机 `3000` 端口服务检查首页 `/`、`/api/photos`、`/api/stories` 均返回 200，且 `/api/photos` 中目标描述为完整 `“我守望着。”`。
- 风险与后续：当前环境没有 Browser 插件，项目内也没有 Playwright CLI，因此未做自动截图级视觉回归；本轮未修改 API、数据字段或存储格式，也未触碰真实上传数据。

## 2026-06-17

- 完成：按用户反馈优化照片描述与故事展示体验。主相册图片 hover 时不再在时间旁边显示描述黑框，仅保留时间信息；灯箱详情页将“编辑信息”拆分为描述、日期、事件、标签、重命名等独立按钮，避免只添加描述时还要连续确认日期和标签。
- 完成：故事模式预览卡片放宽节点间距、画布高度和正文展示规则，描述不再被 2 行或 3 行截断；故事预览中点击图片只编辑描述，不再串联日期、事件和标签输入。
- 完成：故事全屏展示的时间从仅年份改为“年+月”；当照片只有描述时只显示上方高亮文案，不再把同一段描述重复显示两次；hover 时描述位置收回到图片附近，并增强底色以保证可见性。
- 验证：`node --check js/gallery.js`、`node --check js/lightbox.js`、`node --check js/story.js` 通过；`npm run test:smoke` 通过；`npm run audit:data` 通过；临时启动 `node server.js` 后首页 `/` 与 `/api/photos` 均返回 200，`/api/photos` 返回 JSON。
- 风险与后续：本轮未修改 API、数据字段或存储格式，也未触碰真实上传数据；当前环境未安装 Playwright/Browser 插件，未做自动化截图级视觉回归，后续如继续细调故事全屏视觉建议补一轮真实浏览器截图确认。
...
- 完成：修复主相册多选操作条在浅色和深色主题下文字与背景对比不足的问题；为批量操作条、批量整理按钮、全选按钮、取消按钮和禁用态统一收口为更稳定的高对比配色，避免进入多选后整排操作项看不清。
- 完成：为灯箱详情页补上“加入故事”按钮，位置与“设为分组封面 / 编辑信息”保持同组；逻辑直接复用主相册卡片已有的 `promptAddPhotoToStory()` 流程，因此多个故事时仍会先弹出故事选择，新建故事后也能继续加入，并保留“加入成功 / 该图已添加”的内联提示与顶部状态通知。
- 完成：修复多选状态翻页后丢失的问题；批量选择现在同步到当前筛选结果全集，不再因当前分页变化清除已选照片。手动取消批量模式、批量删除/整理/分组完成后仍会清空选择；“全选当前结果”文案同步调整为“全选本页 / 取消本页全选”，避免跨页选择时产生误解。
- 验证：执行 `node --check js/lightbox.js`、`node --check js/story.js`、`node --check js/gallery.js`、`node --check js/main.js` 通过；执行 `npm run test:smoke` 通过。
- 风险与后续：本轮只补灯箱详情入口和批量选择状态，没有改故事数据结构或 API；批量操作条的对比度已针对当前主题体系单独加局部变量，后续如果再大幅调整全局主题变量，建议优先回归检查 `.batch-bar`、跨页多选和灯箱详情操作区。

## 2026-06-18

- 完成：修复故事模式预览卡片描述重复显示的问题，仅保留颜色更深的标题式描述行。
- 完成：优化故事全屏展示的图片说明样式，移除说明文字背后的半透明黑框，收薄默认粉色光边，并缩短图片悬停放大后上下说明文字的偏移距离，降低被视口裁切的概率。
- 验证：执行 `node --check js/story.js`、`npm run test:smoke` 通过；复用本机 `3000` 端口服务检查首页 `/` 返回 200 且包含故事工作区，`/api/photos` 返回 200 和 JSON。
- 风险与后续：本机 Playwright CLI 存在但 Chromium headless 二进制缺失，截图级视觉验证未执行；如需继续精修故事全屏视觉，建议先执行 `npx playwright install` 后再做桌面与移动视口截图回归。
