...
| `index.html` | 页面结构、主要交互区域、故事全屏展示容器和前端模块入口。 |
| `style.css` | 全站视觉系统、布局、响应式、主题、状态样式和故事全屏海报式视觉层。 |
...
| `js/lightbox.js` | 大图查看、切换、编辑、滤镜、设置分组封面、单张照片描述/日期/事件/标签编辑。 |
| `js/upload.js` | 上传入口、拖拽、预览、原图优先上传、高质量处理和上传进度。 |
| `js/select.js` | 轻量自定义下拉增强层，保留原生 `select` 状态源并统一下拉视觉。 |
| `js/story.js` | 故事列表、普通故事时间线、故事内容编辑、节点拖拽布局、故事预览单图描述编辑、全屏展示时间线、节点文案生成、自动播放速度、全屏可编辑文案栏和浏览器本地位置记忆。 |
| `js/comments.js` | 评论和表情回应。 |
| `js/delete.js` | 删除密码弹窗和删除流程。 |
| `js/theme.js` | 明暗模式、背景渐变和主题包。 |
| `js/profile.js` | 昵称、浏览器本地头像和用户徽章。 |
| `js/feedback.js` | 状态提示和轻量反馈。 |
| `js/utils.js` | 标签、日期、HTML 转义、图片尺寸处理和高保真上传准备工具。 |
...
| 路径 | 职责 |
| --- | --- |
| `assets/story-decor/` | 故事全屏展示使用的本地装饰素材目录，通过 `/assets` 静态路径访问；当前包含 Openclipart 来源的樱花、胶片和纸张 SVG；该目录只放 UI 装饰资源，不存放用户上传照片。 |
